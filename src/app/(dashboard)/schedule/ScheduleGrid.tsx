...
import { formatTime, shiftDurationHours } from '@/lib/schedule-types'
...
type ConfirmMode = 'selected' | 'week' | null

export default function ScheduleGrid({ shifts, weekStart, isManager, profiles, stores }: Props) {
  ...
  const visibleShifts = useMemo(() => {
    return shifts.filter(s => {
      if (filterEmployee && s.profileId !== filterEmployee) return false
  ...
  }, [shifts, filterEmployee, filterTimeFrom, filterTimeTo, hiddenDays])
  ...
  const hasFilters = filterEmployee || filterTimeFrom || filterTimeTo || hiddenDays.size > 0
  const allVisibleIds = visibleShifts.map(s => s.id)
  ...
  function executeDelete() {
    ...
    startTransition(async () => {
      const ids = confirmMode === 'selected'
        ? [...selectedIds]
        : allVisibleIds   // 'week' deletes all VISIBLE shifts (respects filters)

      // If clearing the whole week with no filters, use the bulk week action
      const result = confirmMode === 'week' && !hasFilters
    ...
    <div className="space-y-4">
      ...
      {isManager && (
        <div className="card p-4 space-y-3">
          ...
          <div className="flex items-center gap-2 flex-wrap border-t border-stone-100 pt-3">
            ...
            {selectMode && (
              <>
                <button type="button" onClick={selectAll}   className="text-xs text-stone-500 hover:text-stone-900 underline underline-offset-2">All visible</button>
                <button type="button" onClick={clearSelection} className="text-xs text-stone-500 hover:text-stone-900 underline underline-offset-2">None</button>
                {selectedIds.size > 0 && (
                  <button
                    type="button"
                    onClick={() => openConfirm('selected')}
                    className="text-xs px-3 py-1.5 rounded-lg bg-red-600 text-white hover:bg-red-700 transition-colors ml-1"
                  >
            ...
            <div className="ml-auto">
              <button
                type="button"
                onClick={() => openConfirm('week')}
                disabled={shifts.length === 0}
                className="text-xs text-red-600 hover:text-red-800 hover:underline disabled:text-stone-300 disabled:no-underline"
              >
                {hasFilters ? `Clear ${allVisibleIds.length} visible` : 'Clear entire week'}
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-7 gap-2 min-w-0">
        {DAY_NAMES.map((dayName, dayIdx) => {
          ...
          return (
            <div key={dayIdx} className={`min-w-0 transition-opacity ${isHidden ? 'opacity-25 pointer-events-none' : ''}`}>
              {/* Day header */}
                  ...
                  dayShifts.map(shift => {
                    ...
                    return (
                      <div
                        ...
                        className={`relative rounded-md border px-2 py-1.5 text-xs shadow-sm transition-colors ${
                          selectMode ? 'cursor-pointer select-none' : ''
                        } ${
                          isSelected
                            ? 'border-terracotta bg-terracotta/5'
                            : 'border-stone-200 bg-white hover:border-terracotta/40'
                        }`}
                      >
                        {/* Checkbox */}
                        {selectMode && (
                        ...
                        <p className={`font-medium text-stone-800 leading-tight ${selectMode ? 'pr-5' : ''} truncate`}>
                          {shift.profileName}
                        </p>
                        ...
                        {shift.position && (
                          <span className="badge bg-stone-100 text-stone-500 text-[9px] mt-0.5">
                            {shift.position}
                          </span>
                        )}
